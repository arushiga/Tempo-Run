                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            HStack {
                                ...
                                Text(run.distanceMiles == 0 ? "—" : String(format: "%.1f mi", run.distanceMiles))
                                    .font(.caption.weight(.semibold)).foregroundStyle(TempoColor.primary)
                            }
                            Slider(
                                value: Binding(
...
struct PlannerCellView: View {
    ...
    var body: some View {
        ...
        .onTapGesture { if let scheduledRun { viewModel.removeRun(id: scheduledRun.id) } }
    }

    private func cellBackground(for run: ScheduledRun?) -> Color {
        if let run { return run.type.color.opacity(0.12) }
        return isTargeted ? TempoColor.primary.opacity(0.1) : .white.opacity(0.9)
    }

    private func cellBorder(for run: ScheduledRun?) -> Color {
        if isTargeted { return TempoColor.primary }
        if let run { return run.type.color }
